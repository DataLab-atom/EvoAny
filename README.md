...
提供的 MCP tools（共 34 个）：

**演化核心（14）：** `evo_init`、`evo_register_targets`、`evo_report_seed`、`evo_step`、`evo_next_batch`、`evo_report_fitness`、`evo_select_survivors`、`evo_revalidate_targets`、`evo_get_status`、`evo_get_lineage`、`evo_freeze_target`、`evo_boost_target`、`evo_record_synergy`、`evo_check_cache`

**文献与知识（4）：** `lit_ingest`、`lit_search_local`、`code_qa`、`bib_append`

**可视化（3）：** `viz_generate`、`viz_highlight`、`viz_polish`

**基准测试（3）：** `bench_adapt`、`bench_run`、`bench_validate`

**研究推导森林（10）：** `research_init_forest`、`research_add_node`、`research_update_node`、`research_merge_nodes`、`research_check_convergence`、`research_add_convergence_point`、`research_verify_convergence_point`、`research_record_contribution`、`research_get_forest`、`research_iterate`
...
| `/search-lit <query>` | 在本地文献库中搜索相关论文 |
| `/ask-lit <question>` | 基于已收录文献回答研究问题 |
| `/research-loop` | 启动研究推导森林的迭代循环 |
